...
export function LightEffectEditor({ effect, items, onSave, onChange, onDelete }: { effect: LightEffectDefinitionV1; onSave: () => void; onChange: (update: (effect: LightEffectDefinitionV1) => LightEffectDefinitionV1) => void; onDelete: () => void; items: Item[]; providerEnabled: boolean }) {
  ...
  return <div className="effect-card"><div className="section-title"><EditableTitle value={effect.name} fallback={title} ariaLabel={`Rename ${title}`} onChange={(name) => onChange((value) => ({ ...value, name }))} /><div className="effect-header-actions"><button className="mini-icon" onClick={onSave}><SaveToBookIcon /></button><label className="toggle"><input type="checkbox" aria-label={`Enable ${title}`} checked={effect.enabled} onChange={(event) => onChange((value) => ({ ...value, enabled: event.target.checked }))} /></label><button className="mini-icon danger" onClick={onDelete}><TrashIcon /></button></div></div><div className="form-grid">
    ...
    <label title={effect.action === "spotlight" ? "Object whose attached light rotates toward the detected emitter." : undefined}><Label>{effect.action === "spotlight" ? "Light target" : "Target"}</Label><select value={effect.target.type} onChange={(event) => setTarget(event.target.value as EffectTargetV1["type"])}><option value="detector">Self</option><option value="parent">Parent</option><option value="carrier">Carrier</option>{effect.action !== "spotlight" && <option value="detected-emitter">Detected emitter(s)</option>}<option value="specific-item">Specific item</option></select></label>
