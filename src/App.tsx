...
import { ACTION_HEIGHT_STORAGE_KEY, AUTHORITY_CONTROL_CHANNEL, AUTHORITY_STATUS_CHANNEL, DETECTOR_KEY, EFFECT_LIBRARY_STORAGE_KEY, EFFECTS_ENABLED_STORAGE_KEY, EMANATION_INTEGRATION_KEY, EMITTER_KEY, EXTENSION_NAME, RULE_LIBRARY_STORAGE_KEY, RUMBLE_INTEGRATION_KEY, SETTINGS_KEY } from "./constants";
...
import { NumericLimitsProvider } from "./components/NumericLimits";
...
import { LayerCheckboxDropdown } from "./components/LayerCheckboxDropdown";
import { clearEmitterLabels, labelAllEmitters } from "./runtime/emitterLabels";
import { clearDetectorLabels, labelAllDetectors } from "./runtime/detectorLabels";
...
import { pickedRuleValue, PickSceneItemButton, SpecificItemField } from "./components/ItemPickerField";
import { cancelItemPick, completeItemPickFromSelection, isItemPickActive } from "./runtime/itemPicker";

const newEffect = (): ShaderEffectDefinitionV1 => ({ id: crypto.randomUUID(), type: "shader", enabled: true, target: { type: "detector" }, audience: { type: "everyone" }, preset: "glow", shape: "circle", placement: "above", color: "#55aaff", maxIntensity: 1, spread: 1.25, glow: { segments: 1, segmentAlignment: "center" }, animation: { mode: "none", rate: 1, depth: 0.35 } });
...
export const newRule = (): DetectionRuleV1 => ({ id: crypto.randomUUID(), enabled: true, signal: "", detectionArea: "distance", aggregation: "nearest", ignoreHidden: false, disableWhenHidden: false, matchType: "exact", excludeLayers: [], range: { outer: 60, inner: 0 }, falloff: "smoothstep", effects: [newEffect()] });
...
const EditableGlyph = ({ kind }: { kind: "gears" | "book" | "light" }) => <span className={`editable-glyph editable-glyph-${kind}`} aria-hidden="true" />;
const VisualEffectGlyph = () => <span className="editable-visual-glyph" aria-hidden="true" />;
...
const effectDefaultName = (effect: EffectDefinitionV1) => effect.type === "shader" ? effect.preset === "beam" ? "Directional beam" : effect.preset === "radar" ? "Radar" : effect.preset === "grid" ? "Grid" : effect.preset === "edge" ? "Edge Indicator" : "Glow/Shadow" : effect.type === "mechanical" ? stateEffectName(effect) : effect.type === "light" ? effect.action === "add" ? "Add Light" : effect.action === "modify" ? "Modify Light" : "Spotlight" : INTEGRATION_CATALOG.find((provider) => provider.id === effect.providerId)?.displayName ?? "Integration effect";
const effectTypeLabel = (effect: EffectDefinitionV1) => effect.type === "shader" ? effect.preset === "glow" ? "Glow/Shadow" : effect.preset === "radar" ? "Radar" : effect.preset === "grid" ? "Grid" : effect.preset === "edge" ? "Edge Indicator" : "Directional Beam" : effect.type === "mechanical" ? stateEffectName(effect) : effect.type === "light" ? effect.action === "add" ? "Add Light" : effect.action === "modify" ? "Modify Light" : "Spotlight" : INTEGRATION_CATALOG.find((provider) => provider.id === effect.providerId)?.displayName ?? effect.providerId;
...
export default function App() {
  ...
  const [detectorLabelsVisible, setDetectorLabelsVisible] = useState(false);
  const [detectorLabelCount, setDetectorLabelCount] = useState(0);
  ...
  const [effectsEnabled, setEffectsEnabled] = useState(() => localStorage.getItem(EFFECTS_ENABLED_STORAGE_KEY) !== "false");
  ...
  const loadSelection = useCallback(async (nextItems?: Item[]) => {
    ...
    if (isItemPickActive()) completeItemPickFromSelection();
    const selection = isItemPickActive() ? (selectedIdRef.current ? [selectedIdRef.current] : undefined) : playerSelection;
  ...
  useEffect(() => {
    const syncEffectsEnabled = (event: StorageEvent) => { if (event.key === EFFECTS_ENABLED_STORAGE_KEY) setEffectsEnabled(event.newValue !== "false"); };
    window.addEventListener("storage", syncEffectsEnabled);
    return () => window.removeEventListener("storage", syncEffectsEnabled);
  }, []);

  useEffect(() => { if (connection.role !== "GM") setShowDebug(false); }, [connection.role]);
  ...
  useEffect(() => {
    ...
    setDetectorLabelCount(0);
    void Promise.all([clearEmitterLabels(), clearDetectorLabels(), cancelItemPick()]);
  }, [connection.sceneReady]);

  useEffect(() => () => { void clearEmitterLabels(); void clearDetectorLabels(); void cancelItemPick(); }, []);
  ...
  const toggleDetectorLabels = () => {
    if (detectorLabelsVisible) {
      void clearDetectorLabels().then(() => { setDetectorLabelsVisible(false); setDetectorLabelCount(0); });
      return;
    }
    void labelAllDetectors(items, debug).then((count) => { setDetectorLabelsVisible(count > 0); setDetectorLabelCount(count); });
  };
  const toggleEffects = () => {
    const enabled = !effectsEnabled;
    localStorage.setItem(EFFECTS_ENABLED_STORAGE_KEY, String(enabled));
    setEffectsEnabled(enabled);
  };
  const updateSceneSettings = (next: SceneSettingsV1) => {
    if (connection.role !== "GM" || !connection.sceneReady) return;
  ...
  const updateDistanceMethod = (distanceMethod: SceneSettingsV1["distanceMethod"]) => updateSceneSettings({ ...sceneSettings, distanceMethod });
  const updateAllowOutOfBounds = (allowOutOfBounds: boolean) => updateSceneSettings({ ...sceneSettings, allowOutOfBounds });
  ...
  useEffect(() => {
    ...
    if (detector.rules.length && !normalizedDetector) {
      setSaveError("Fix invalid settings: signals are required and range or radius inner values must not exceed their outer values.");
  ...
  return <NumericLimitsProvider value={connection.role === "GM" && sceneSettings.allowOutOfBounds === true}><main className="app-shell">
    <header className="app-header"><div className="brand"><img src="./icon.svg" alt="" /><h1>{EXTENSION_NAME}</h1></div><nav className="header-actions" aria-label="Extension controls"><button className={`effects-stop-button ${effectsEnabled ? "stop" : "start"}`} title={effectsEnabled ? "Immediately disable all Sting effects in this browser." : "Resume Sting effects in this browser."} aria-label={effectsEnabled ? "Stop all effects" : "Start all effects"} aria-pressed={!effectsEnabled} onClick={toggleEffects}><span className="effects-stop-light" aria-hidden="true" />{effectsEnabled ? "STOP" : "START"}</button>{connection.role === "GM" && <button className={`icon-button${showDebug ? " active" : ""}`} title="Debug runtime" aria-label="Debug runtime" onClick={() => { setShowDebug((value) => !value); setShowSettings(false); }}><BugIcon /></button>}<button className={`icon-button${showSettings ? " active" : ""}`} title="Settings" aria-label="Settings" onClick={() => { setShowSettings((value) => !value); setShowDebug(false); }}><GearsIcon /></button><a className="icon-button" href={extensionRoot} target="_blank" rel="noreferrer" title="Open Sting help" aria-label="Open Sting help">?</a></nav></header>
    {!connection.sceneReady && <div className="notice">Open a scene to configure proximity signals.</div>}
    {connection.sceneReady && connection.role !== "GM" && <div className="notice">{effectsEnabled ? "The runtime is active." : "Effects are stopped in this browser."} Only the GM can configure scene items.</div>}
    {connection.sceneReady && connection.role === "GM" && !selected && <div className="notice">Select exactly one scene item to configure it.</div>}
    {connection.status === "ready" && showSettings && <section className="content-card settings-section"><div className="section-title"><div><h2>Settings</h2>{connection.role === "GM" && <p className="muted">Distance settings for this scene</p>}</div></div>
      {connection.role === "GM" ? <div className="settings-content">
        ...
        <div className="settings-subsection"><div className="extension-row"><div><strong>Override numerical limits</strong><p className="muted">Allow direct entry of values outside the normal bounds for every numerical setting.</p></div><label className="toggle" title="Allow numerical settings outside their normal bounds."><input type="checkbox" aria-label="Override numerical limits" checked={sceneSettings.allowOutOfBounds === true} disabled={!connection.sceneReady} onChange={(event) => updateAllowOutOfBounds(event.target.checked)} /></label></div>{sceneSettings.allowOutOfBounds && <p className="numeric-limits-warning" role="alert">Warning: Settings outside normal bounds could cause unexpected behavior, including crashes.</p>}</div>
      ...
      </div> : <p className="muted player-settings-placeholder">No player settings are currently available.</p>}
    </section>}
    {showDebug && connection.role === "GM" ? <><div className="debug-label-controls"><button className="wide-button" disabled={!connection.sceneReady} onClick={toggleEmitterLabels}>{emitterLabelsVisible ? "Clear emitter labels" : "Label all emitters"}</button>{emitterLabelsVisible && <p className="muted" role="status">Showing {emitterLabelCount} local emitter label{emitterLabelCount === 1 ? "" : "s"}.</p>}<button className="wide-button" disabled={!connection.sceneReady} onClick={toggleDetectorLabels}>{detectorLabelsVisible ? "Clear detector labels" : "Label all detectors"}</button>{detectorLabelsVisible && <p className="muted" role="status">Showing {detectorLabelCount} local detector label{detectorLabelCount === 1 ? "" : "s"}.</p>}</div><DebugView rules={debug} authority={authority} pending={authorityPending} error={authorityError} onTakeControl={() => sendAuthorityCommand("take-control")} onReleaseControl={() => sendAuthorityCommand("release-control")} /></> : selected && !showSettings && connection.role === "GM" ? <>
...
function RuleEditor({ rule, index, unit, items, party, emanationEnabled, library, onSaveRule, onSaveEffect, onChange, onEffect, onDelete }: RuleEditorProps) {
  const [sourcePickError, setSourcePickError] = useState<string | null>(null);
  ...
  return <article className="rule-card"><div className="section-title"><div className="card-heading-main"><button className="collapse-button" title={collapsed ? "Expand rule" : "Collapse rule"} aria-label={collapsed ? `Expand rule ${index + 1}` : `Collapse rule ${index + 1}`} aria-expanded={!collapsed} onClick={() => setCollapsed((value) => !value)}><CaretIcon collapsed={collapsed} /></button><EditableTitle as="h3" value={rule.name} fallback={`Rule ${index + 1}`} ariaLabel={`Rename rule ${index + 1}`} onChange={(name) => onChange((value) => ({ ...value, name }))} /></div><div className="rule-header-actions"><button className="mini-icon" title="Save this rule and all of its effects to your browser-local library." aria-label={`Save rule ${index + 1} to library`} onClick={onSaveRule}><SaveToBookIcon /></button><label className="toggle" title="Enable or disable this detection rule."><input type="checkbox" aria-label={`Enable rule ${index + 1}`} checked={rule.enabled} onChange={(event) => onChange((value) => ({ ...value, enabled: event.target.checked }))} /></label><button className="mini-icon danger delete-icon" title="Delete this detection rule." aria-label={`Delete rule ${index + 1}`} onClick={onDelete}><TrashIcon /></button></div></div>{!collapsed && <><div className="form-grid">
    <label title="Choose the kind of source this detector observes."><Label>Detection Type</Label><select value={rule.source?.type ?? "sting-emitter"} onChange={(event) => onChange((value) => ({ ...value, source: event.target.value === "obr-light" ? { type: "obr-light", ownership: "any", attachment: "any" } : { type: event.target.value as "sting-emitter" | "item-name" | "item-label" } }))}><option value="sting-emitter">Emitter Tag</option><option value="item-name">Item Name</option><option value="item-label">Item Label</option><option value="obr-light">OBR Light</option></select></label>
    {rule.source?.type !== "obr-light" && <div className="item-match-field" title="Value this rule matches after case and spacing normalization."><Label>{rule.source?.type === "item-name" ? "Item Name" : rule.source?.type === "item-label" ? "Item Label" : "Signal"}</Label><div className="item-picker-row"><input aria-label={rule.source?.type === "item-name" ? "Item Name" : rule.source?.type === "item-label" ? "Item Label" : "Signal"} value={rule.signal} placeholder={rule.source?.type === "item-name" ? "Add item name…" : rule.source?.type === "item-label" ? "Add item label…" : "Add signal…"} onChange={(event) => { setSourcePickError(null); onChange((value) => ({ ...value, signal: event.target.value })); }} />{(rule.source?.type === "item-name" || rule.source?.type === "item-label") && <PickSceneItemButton items={items} onError={setSourcePickError} onPick={(item) => { const sourceType = rule.source?.type as "item-name" | "item-label"; const signal = pickedRuleValue(item, sourceType); if (!signal) return sourceType === "item-name" ? "Selected item has no name." : "Selected item has no qualifying label text."; onChange((value) => ({ ...value, signal })); }} />}</div>{sourcePickError && <small className="field-error" role="alert">{sourcePickError}</small>}</div>}
    <label title="Choose the geometry used to qualify matching items."><Label tooltip={rule.source?.type === "obr-light" ? "Use Sting's configured range or the OBR light's geometric area. Walls and shadows are not considered." : "Use Sting's configured range or the detected item's bounds."}>Detection Area</Label><select value={rule.detectionArea} onChange={(event) => onChange((value) => ({ ...value, detectionArea: event.target.value as DetectionRuleV1["detectionArea"] }))}><option value="distance">Detection range</option><option value="source-area">{rule.source?.type === "obr-light" ? "Within light area" : "Within item bounds"}</option></select></label>
    {rule.detectionArea === "distance" && <label title="How effect strength changes between the full-strength and outer ranges."><Label tooltip="How effect strength changes between the full-strength and outer ranges.">Falloff</Label><select value={rule.falloff} onChange={(event) => onChange((value) => ({ ...value, falloff: event.target.value as DetectionRuleV1["falloff"] }))}><option value="smoothstep">Smooth</option><option value="linear">Linear</option><option value="logarithmic">Logarithmic</option><option value="binary">Binary</option></select></label>}
    <label title="Choose whether effects receive the closest qualifying item or every qualifying item."><Label tooltip="Choose the closest qualifying detected item or all qualifying detected items.">Match Selection</Label><select value={rule.aggregation} onChange={(event) => onChange((value) => ({ ...value, aggregation: event.target.value as DetectionRuleV1["aggregation"] }))}><option value="nearest">Closest detected item</option><option value="all">All detected items</option></select></label>
    ...
    <details className="advanced-section rule-advanced wide"><summary>Advanced rule settings</summary><div className="advanced-grid">
      ...
      <LayerCheckboxDropdown className={rule.source?.type === "obr-light" ? "wide" : undefined} layers={OBR_LAYERS} value={rule.excludeLayers} onChange={(excludeLayers) => onChange((value) => ({ ...value, excludeLayers }))} />
    </div></details>
  </div><div className="effects-heading"><h4>Effects <span className="effect-count">{rule.effects.length}</span></h4><div className="effect-add-actions" aria-label="Add effect"><button className="effect-glyph" title="Add visual effect" aria-label="Add visual effect" onClick={() => onChange((value) => ({ ...value, effects: [...value.effects, newEffect()] }))}><VisualEffectGlyph /></button><button className="effect-glyph" title="Add state effect" aria-label="Add state effect" onClick={() => onChange((value) => ({ ...value, effects: [...value.effects, newFaceEffect()] }))}><EditableGlyph kind="gears" /></button><button className="effect-glyph" title="Add light effect" aria-label="Add light effect" onClick={() => onChange((value) => ({ ...value, effects: [...value.effects, newLightEffect()] }))}><EditableGlyph kind="light" /></button><button className={`effect-glyph${libraryOpen ? " active" : ""}`} title={library.length ? "Add an effect from your browser-local library." : "Your browser-local effects library is empty."} aria-label="Add an effect from the effects library" disabled={!library.length} onClick={() => setLibraryOpen((value) => !value)}><EditableGlyph kind="book" /></button>{enabledProviders.map((provider) => <button className="effect-glyph" key={provider.id} title={`Add ${provider.displayName} effect.`} aria-label={`Add ${provider.displayName} effect`} onClick={() => onChange((value) => ({ ...value, effects: [...value.effects, createIntegrationEffect(provider.id, provider.actions[0].id)] }))}><img src={provider.iconUrl} alt="" /></button>)}</div></div>{libraryOpen && <div className="library-picker">{library.map((entry) => <button key={entry.id} title={`Add ${entry.name} to this rule.`} onClick={() => { onChange((value) => ({ ...value, effects: [...value.effects, instantiateLibraryEffect(entry)] })); setLibraryOpen(false); }}><span>{entry.name}</span><small>{effectTypeLabel(entry.effect)}</small></button>)}</div>}{rule.effects.map((effect, effectIndex) => { const deleteEffect = () => confirmDelete(`Delete effect ${effectIndex + 1} from Rule ${index + 1}?`, () => onChange((value) => ({ ...value, effects: value.effects.filter((_, i) => i !== effectIndex) }))); return effect.type === "shader" ? <EffectEditor key={effect.id} effect={effect} items={items} party={party} onSave={() => onSaveEffect(effect)} onChange={(update) => onEffect(effectIndex, (current) => current.type === "shader" ? update(current) : current)} onDelete={deleteEffect} /> : effect.type === "mechanical" ? <MechanicalEffectEditor key={effect.id} effect={effect} items={items} onSave={() => onSaveEffect(effect)} onChange={(update) => onEffect(effectIndex, (current) => current.type === "mechanical" ? update(current) : current)} onDelete={deleteEffect} /> : <IntegrationEffectEditor key={effect.id} effect={effect} items={items} providerEnabled={emanationEnabled} onSave={() => onSaveEffect(effect)} onChange={(update) => onEffect(effectIndex, (current) => (current.type === "integration" || current.type === "light") ? update(current) : current)} onDelete={deleteEffect} />; })}</>}</article>;
}
...
function MechanicalEffectEditor({ effect, items, onSave, onChange, onDelete }: { effect: MechanicalEffectDefinitionV1; items: Item[]; onSave: () => void; onChange: (update: (effect: MechanicalEffectDefinitionV1) => MechanicalEffectDefinitionV1) => void; onDelete: () => void }) {
  ...
  return <div className="effect-card"><div className="section-title"><div className="card-heading-main"><button className="collapse-button" title={collapsed ? "Expand effect" : "Collapse effect"} aria-label={collapsed ? "Expand Face effect" : "Collapse Face effect"} aria-expanded={!collapsed} onClick={() => setCollapsed((value) => !value)}><CaretIcon collapsed={collapsed} /></button><EditableTitle value={effect.name} fallback="Face" ariaLabel="Rename Face effect" onChange={(name) => onChange((value) => ({ ...value, name }))} /></div><div className="effect-header-actions"><button className="mini-icon" title="Save this effect to your browser-local library." aria-label="Save effect to library" onClick={onSave}><SaveToBookIcon /></button><label className="toggle" title="Enable or disable this effect."><input type="checkbox" aria-label="Enable Face effect" checked={effect.enabled} onChange={(event) => onChange((value) => ({ ...value, enabled: event.target.checked }))} /></label><button className="mini-icon danger delete-icon" title="Delete effect" aria-label="Delete Face effect" onClick={onDelete}><TrashIcon /></button></div></div>{!collapsed && <div className="form-grid">
    ...
    <label className="wide" title="Scene item that rotates to face the closest emitter."><Label tooltip="Scene item that rotates to face the closest emitter.">Effect target</Label><select value={effect.target.type} onChange={(event) => setTarget(event.target.value as EffectTargetV1["type"])}><option value="detector">Self</option><option value="parent">Parent</option><option value="carrier">Carrier</option><option value="detected-emitter">Detected item(s)</option><option value="specific-item">Specific item</option></select></label>
    {effect.target.type === "specific-item" && <SpecificItemField items={items} value={effect.target.itemId} tooltip="Exact scene item to rotate." onChange={(itemId) => onChange((value) => ({ ...value, target: { type: "specific-item", itemId } }))} />}
...
function VisibilityEffectEditor({ effect, items, onSave, onActionChange, onChange, onDelete }: { effect: Extract<MechanicalEffectDefinitionV1, { action: "visibility" }>; items: Item[]; onSave: () => void; onActionChange: (action: MechanicalEffectDefinitionV1["action"]) => void; onChange: (update: (effect: Extract<MechanicalEffectDefinitionV1, { action: "visibility" }>) => Extract<MechanicalEffectDefinitionV1, { action: "visibility" }>) => void; onDelete: () => void }) {
  ...
  return <div className="effect-card"><div className="section-title"><div className="card-heading-main"><button className="collapse-button" title={collapsed ? "Expand effect" : "Collapse effect"} aria-label={collapsed ? "Expand Hide/Show effect" : "Collapse Hide/Show effect"} aria-expanded={!collapsed} onClick={() => setCollapsed((value) => !value)}><CaretIcon collapsed={collapsed} /></button><EditableTitle value={effect.name} fallback="Hide/Show" ariaLabel="Rename Hide/Show effect" onChange={(name) => onChange((value) => ({ ...value, name }))} /></div><div className="effect-header-actions"><button className="mini-icon" title="Save this effect to your browser-local library." aria-label="Save effect to library" onClick={onSave}><SaveToBookIcon /></button><label className="toggle" title="Enable or disable this effect."><input type="checkbox" aria-label="Enable Hide/Show effect" checked={effect.enabled} onChange={(event) => onChange((value) => ({ ...value, enabled: event.target.checked }))} /></label><button className="mini-icon danger delete-icon" title="Delete effect" aria-label="Delete Hide/Show effect" onClick={onDelete}><TrashIcon /></button></div></div>{!collapsed && <div className="form-grid">
    ...
    <label className="wide" title="Scene item whose visibility changes when the threshold is crossed."><Label tooltip="Scene item whose visibility changes when the threshold is crossed.">Effect target</Label><select value={effect.target.type} onChange={(event) => setTarget(event.target.value as EffectTargetV1["type"])}><option value="detector">Self</option><option value="parent">Parent</option><option value="carrier">Carrier</option><option value="detected-emitter">Detected item(s)</option><option value="specific-item">Specific item</option></select></label>
    {effect.target.type === "specific-item" && <SpecificItemField items={items} value={effect.target.itemId} tooltip="Exact scene item whose visibility changes." onChange={(itemId) => onChange((value) => ({ ...value, target: { type: "specific-item", itemId } }))} />}
...
function StateTarget<T extends MechanicalEffectDefinitionV1>({ effect, items, description, onChange }: { effect: T; items: Item[]; description: string; onChange: (update: (effect: T) => T) => void }) {
  const setTarget = (type: EffectTargetV1["type"]) => onChange((value) => ({ ...value, target: type === "specific-item" ? { type, itemId: items[0]?.id ?? "" } : { type } }));
  return <><label className="wide" title={description}><Label tooltip={description}>Effect target</Label><select value={effect.target.type} onChange={(event) => setTarget(event.target.value as EffectTargetV1["type"])}><option value="detector">Self</option><option value="parent">Parent</option><option value="carrier">Carrier</option><option value="detected-emitter">Detected item(s)</option><option value="specific-item">Specific item</option></select></label>{effect.target.type === "specific-item" && <SpecificItemField items={items} value={effect.target.itemId} tooltip="Exact scene item to change." onChange={(itemId) => onChange((value) => ({ ...value, target: { type: "specific-item", itemId } }))} />}</>;
}
...
function EffectEditor({ effect, items, party, onSave, onChange, onDelete }: { effect: ShaderEffectDefinitionV1; items: Item[]; party: Player[]; onSave: () => void; onChange: (update: (effect: ShaderEffectDefinitionV1) => ShaderEffectDefinitionV1) => void; onDelete: () => void }) {
  const [collapsed, setCollapsed] = useState(false);
  const [animationOpen, setAnimationOpen] = useState(effect.preset === "radar" ? (effect.radar?.sweepType ?? "none") !== "none" : effect.animation?.mode !== "none" && effect.animation !== undefined);
  ...
  const setRadar = (update: Partial<NonNullable<ShaderEffectDefinitionV1["radar"]>>) => onChange((current) => ({ ...current, radar: { echoStyle: "circle", echoSize: 100, distanceScale: "linear", decoration: "none", sweepTrail: 0, brightness: 0.35, sweepType: "none", sweepDirection: "outward", echoFadeDuration: 3, ...current.radar, ...update } }));
  const setEdge = (update: Partial<NonNullable<ShaderEffectDefinitionV1["edge"]>>) => onChange((current) => ({ ...current, edge: { appearance: "triangle", orientation: "toward-detection", size: 48, inset: 16, ...current.edge, ...update } }));
  ...
  const fieldIsDynamic = (field: ShaderDynamicField, legacyLink?: StrengthLinkDirection) => {
    const range = effect.dynamicRanges?.[field];
    return range ? range.enabled !== false : legacyLink !== undefined;
  };
  const beamRadiiDynamic = fieldIsDynamic("innerRadius", geometry.innerRadiusStrengthLink) || fieldIsDynamic("outerRadius", geometry.outerRadiusStrengthLink);
  ...
  return <div className="effect-card"><div className="section-title"><div className="card-heading-main"><button className="collapse-button" title={collapsed ? "Expand effect" : "Collapse effect"} aria-label={collapsed ? `Expand ${title} effect` : `Collapse ${title} effect`} aria-expanded={!collapsed} onClick={() => setCollapsed((value) => !value)}><CaretIcon collapsed={collapsed} /></button><EditableTitle value={effect.name} fallback={defaultTitle} ariaLabel={`Rename ${defaultTitle} effect`} onChange={(name) => onChange((value) => ({ ...value, name }))} /></div><div className="effect-header-actions"><button className="mini-icon" title="Save this effect to your browser-local library." aria-label="Save effect to library" onClick={onSave}><SaveToBookIcon /></button><label className="toggle" title="Enable or disable this effect."><input type="checkbox" aria-label={`Enable ${title} effect`} checked={effect.enabled} onChange={(event) => onChange((value) => ({ ...value, enabled: event.target.checked }))} /></label><button className="mini-icon danger delete-icon" title="Delete effect" aria-label={`Delete ${title} effect`} onClick={onDelete}><TrashIcon /></button></div></div>{!collapsed && <><div className="form-grid">
    <div className="paired-controls wide"><label title="Choose the native visual treatment."><Label tooltip="Choose the native visual treatment.">Effect type</Label><select value={effect.preset} onChange={(event) => onChange((value) => { const preset = event.target.value as ShaderEffectDefinitionV1["preset"]; return { ...value, preset, geometry: DEFAULT_GEOMETRY[preset], ...(preset === "glow" && !value.glow ? { glow: { segments: 1, segmentAlignment: "center" as const } } : {}), ...(preset === "radar" && !value.radar ? { radar: { echoStyle: "circle" as const, echoSize: 100, distanceScale: "linear" as const, decoration: "none" as const, sweepTrail: 0, brightness: 0.35, sweepType: "none" as const, sweepDirection: "outward" as const, echoFadeDuration: 3 } } : {}), ...(preset === "grid" && !value.grid ? { grid: { showGrid: false, showImages: false, imageBackgrounds: false } } : {}), ...(preset === "edge" && !value.edge ? { edge: { appearance: "triangle" as const, orientation: "toward-detection" as const, size: 48, inset: 16 } } : {}) }; })}><option value="glow">Glow/Shadow</option><option value="beam">Directional beam</option><option value="radar">Radar</option><option value="grid">Grid</option><option value="edge">Edge Indicator</option></select></label><label title="Scene item used as the effect origin."><Label tooltip="Scene item used as the effect origin.">Effect target</Label><select value={effect.target.type} onChange={(event) => setTarget(event.target.value as EffectTargetV1["type"])}><option value="detector">Self</option><option value="parent">Parent</option><option value="carrier">Carrier</option><option value="detected-emitter">Detected item(s)</option><option value="specific-item">Specific item</option></select></label></div>
    {effect.target.type === "specific-item" && <SpecificItemField items={items} value={effect.target.itemId} tooltip="Exact scene item to target." onChange={(itemId) => onChange((value) => ({ ...value, target: { type: "specific-item", itemId } }))} />}
    ...
    {effect.preset !== "edge" && <div className="paired-controls wide"><label title="Choose the visual effect boundary shape."><Label tooltip="Choose the visual effect boundary shape.">Shape</Label><select value={effect.shape} onChange={(event) => onChange((value) => ({ ...value, shape: event.target.value as ShaderEffectDefinitionV1["shape"] }))}><option value="circle">Circle</option><option value="square">Square</option></select></label><label title="Draw the visual effect immediately above or below its target on the same scene layer."><Label tooltip="Draw the visual effect immediately above or below its target on the same scene layer.">Placement</Label><select value={effect.placement} onChange={(event) => onChange((value) => ({ ...value, placement: event.target.value as ShaderEffectDefinitionV1["placement"] }))}><option value="above">Above target</option><option value="below">Below target</option></select></label></div>}
    ...
    <DynamicSliderNumber tooltip={effect.preset === "radar" ? "Maximum opacity of the sweep and echoes; decoration opacity is independent." : "Effect opacity at minimum and maximum signal strength."} label="Intensity" min={0} max={2} step={0.05} decimals={2} value={effect.maxIntensity} range={rangeFor("intensity", (effect.intensityStrengthLinked ?? true) ? { minimum: 0, maximum: effect.maxIntensity } : undefined)} onChange={(maxIntensity) => onChange((value) => ({ ...value, maxIntensity }))} onRangeChange={(range) => setDynamicRange("intensity", range)} />
    <DynamicSliderNumber tooltip="Feathering at the effect edges; 0 produces crisp edges." label="Softness" min={0} max={4} step={0.05} decimals={2} value={effect.spread} range={rangeFor("softness", legacyRange(effect.spread, effect.spreadStrengthLink, 0, 4))} onChange={(spread) => onChange((value) => ({ ...value, spread }))} onRangeChange={(range) => setDynamicRange("softness", range)} />
    {effect.preset === "edge" && <><div className="paired-controls wide"><label title="Visual used for each offscreen emitter."><Label>Indicator</Label><select value={effect.edge?.appearance ?? "triangle"} onChange={(event) => setEdge({ appearance: event.target.value as NonNullable<ShaderEffectDefinitionV1["edge"]>["appearance"] })}><option value="triangle">Triangle</option><option value="disk">Disk</option><option value="image">Image</option><option value="square">Square</option><option value="target">Target</option><option value="echo">Echo</option><option value="rune">Rune</option><option value="arcane">Arcane</option></select></label><label title="Direction each indicator faces."><Label>Orientation</Label><select value={effect.edge?.orientation ?? "toward-detection"} onChange={(event) => setEdge({ orientation: event.target.value as "toward-edge" | "toward-detection" })}><option value="toward-edge">Toward edge</option><option value="toward-detection">Toward detection</option></select></label></div><div className="paired-controls wide"><DynamicSliderNumber tooltip="Indicator diameter in screen pixels. DYN resolves this separately for each detection." label="Size" min={16} max={160} step={1} suffix=" px" value={effect.edge?.size ?? 48} range={rangeFor("indicatorSize")} onChange={(size) => setEdge({ size })} onRangeChange={(range) => setDynamicRange("indicatorSize", range)} /><SliderNumber tooltip="Distance between the indicator and viewport edge; negative values move it beyond the edge." label="Edge inset" min={-96} max={96} step={1} suffix=" px" value={effect.edge?.inset ?? 16} onChange={(inset) => setEdge({ inset })} /></div></>}
    {effect.preset === "edge" ? null : effect.preset === "beam" ? <div className="paired-controls wide"><DynamicSliderNumber tooltip="Radial distance where the beam begins; 100% is the target edge." label="Beam start" range={rangeFor("innerRadius", legacyRange(geometry.innerRadius, geometry.innerRadiusStrengthLink, 0, Math.max(0, geometry.outerRadius - 1)))} min={0} max={beamRadiiDynamic ? 199 : Math.max(0, geometry.outerRadius - 1)} step={1} value={geometry.innerRadius} suffix="%" onChange={(value) => setGeometry("innerRadius", value)} onRangeChange={(range) => setDynamicRange("innerRadius", range)} /><DynamicSliderNumber tooltip="Radial distance where the beam ends; 100% is the target edge." label="Beam end" range={rangeFor("outerRadius", legacyRange(geometry.outerRadius, geometry.outerRadiusStrengthLink, geometry.innerRadius + 1, 200))} min={beamRadiiDynamic ? 1 : geometry.innerRadius + 1} max={200} step={1} value={geometry.outerRadius} suffix="%" onChange={(value) => setGeometry("outerRadius", value)} onRangeChange={(range) => setDynamicRange("outerRadius", range)} /></div> : <><DynamicSliderNumber tooltip="Inner edge of the glow; 100% is the target edge." label="Inner radius" range={rangeFor("innerRadius", legacyRange(geometry.innerRadius, geometry.innerRadiusStrengthLink, 0, Math.max(0, geometry.outerRadius - 1)))} min={0} max={Math.max(0, geometry.outerRadius - 1)} step={1} value={geometry.innerRadius} suffix="%" onChange={(value) => setGeometry("innerRadius", value)} onRangeChange={(range) => setDynamicRange("innerRadius", range)} /><DynamicSliderNumber tooltip="Outer edge of the glow; 100% is the target edge." label="Outer radius" range={rangeFor("outerRadius", legacyRange(geometry.outerRadius, geometry.outerRadiusStrengthLink, geometry.innerRadius + 1, 200))} min={geometry.innerRadius + 1} max={200} step={1} value={geometry.outerRadius} suffix="%" onChange={(value) => setGeometry("outerRadius", value)} onRangeChange={(range) => setDynamicRange("outerRadius", range)} /></>}
    {effect.preset === "beam" && <div className="paired-controls wide"><DynamicSliderNumber tooltip="Angular width at Beam end." label="End width" range={rangeFor("beamWidth", legacyRange(effect.beamWidth ?? 38, effect.beamWidthStrengthLink, 0, 120))} min={0} max={120} step={1} value={effect.beamWidth ?? 38} suffix="°" onChange={(beamWidth) => onChange((value) => ({ ...value, beamWidth }))} onRangeChange={(range) => setDynamicRange("beamWidth", range)} /><DynamicSliderNumber tooltip="Width at the effect center as a percentage of the target width." label="Origin width" range={rangeFor("beamOriginWidth")} min={0} max={100} step={1} value={effect.beamOriginWidth ?? 0} suffix="%" onChange={(beamOriginWidth) => onChange((value) => ({ ...value, beamOriginWidth }))} onRangeChange={(range) => setDynamicRange("beamOriginWidth", range)} /></div>}
    {effect.preset === "glow" && <div className="paired-controls wide"><DynamicSliderNumber tooltip="Divide the Glow/Shadow into fixed, equal angular wedges. DYN resolves to a whole segment count for each detection." label="Segments" min={1} max={30} step={1} value={effect.glow?.segments ?? 1} range={rangeFor("segments")} onChange={(segments) => onChange((value) => ({ ...value, glow: { segments, segmentAlignment: value.glow?.segmentAlignment ?? "center" } }))} onRangeChange={(range) => setDynamicRange("segments", range)} /><label title="Choose whether 0° (straight up) is the center of a segment or a dividing line."><Label>At 0°</Label><select value={effect.glow?.segmentAlignment ?? "center"} onChange={(event) => onChange((value) => ({ ...value, glow: { segments: value.glow?.segments ?? 1, segmentAlignment: event.target.value as "center" | "boundary" } }))}><option value="center">Segment center</option><option value="boundary">Segment boundary</option></select></label></div>}
    {effect.preset === "radar" && <>
      <div className="paired-controls wide"><label title="Visual treatment for detected emitters."><Label>Echo style</Label><select value={effect.radar?.echoStyle ?? "circle"} onChange={(event) => setRadar({ echoStyle: event.target.value as "circle" | "blob" | "rune" })}><option value="circle">Disk</option><option value="blob">Blob</option><option value="rune">Rune</option></select></label><DynamicSliderNumber tooltip="Mix the echoes from the selected color toward white." label="Brightness" min={0} max={1} step={0.05} decimals={2} value={effect.radar?.brightness ?? 0.35} range={rangeFor("radarBrightness")} onChange={(brightness) => setRadar({ brightness })} onRangeChange={(range) => setDynamicRange("radarBrightness", range)} /></div>
      <DynamicSliderNumber className="wide" tooltip="Base echo size; detected item size remains a relative multiplier. DYN resolves this base separately for each detection." label="Echo size" min={10} max={400} step={5} suffix="%" value={effect.radar?.echoSize ?? 100} range={rangeFor("radarEchoSize")} onChange={(echoSize) => setRadar({ echoSize })} onRangeChange={(range) => setDynamicRange("radarEchoSize", range)} />
      <div className="paired-controls wide"><label title="How scene distance is distributed between the inner and outer radar radii."><Label>Distance scale</Label><select value={effect.radar?.distanceScale ?? "linear"} onChange={(event) => setRadar({ distanceScale: event.target.value as "linear" | "logarithmic" })}><option value="linear">Linear</option><option value="logarithmic">Logarithmic</option></select></label><label title="Optional decorative line work drawn over the radar display."><Label>Decoration</Label><select value={effect.radar?.decoration ?? "none"} onChange={(event) => setRadar({ decoration: event.target.value as "none" | "m314" | "modern" | "arcane" })}><option value="none">None</option><option value="m314">M314</option><option value="modern">Modern</option><option value="arcane">Arcane</option></select></label></div>
    </>}
    {effect.preset === "grid" && <><div className="paired-controls wide"><label className="checkbox-field" title="Draw a thin, world-aligned copy of the scene grid."><Label>Show grid</Label><span className="toggle"><input type="checkbox" aria-label="Show grid" checked={effect.grid?.showGrid ?? false} onChange={(event) => onChange((value) => ({ ...value, grid: { showGrid: event.target.checked, showImages: value.grid?.showImages ?? false, imageBackgrounds: value.grid?.imageBackgrounds ?? false } }))} /></span></label><label className="checkbox-field" title="Show detected image artwork at its projected Grid position and size."><Label>Show images</Label><span className="toggle"><input type="checkbox" aria-label="Show images" checked={effect.grid?.showImages ?? false} onChange={(event) => onChange((value) => ({ ...value, grid: { showGrid: value.grid?.showGrid ?? false, showImages: event.target.checked, imageBackgrounds: value.grid?.imageBackgrounds ?? false } }))} /></span></label></div>{(effect.grid?.showImages ?? false) && <label className="checkbox-field wide" title="Draw a solid, gradient-resolved square behind each detected image."><Label>Image backgrounds</Label><span className="toggle"><input type="checkbox" aria-label="Image backgrounds" checked={effect.grid?.imageBackgrounds ?? false} onChange={(event) => onChange((value) => ({ ...value, grid: { showGrid: value.grid?.showGrid ?? false, showImages: value.grid?.showImages ?? false, imageBackgrounds: event.target.checked } }))} /></span></label>}</>}
    {effect.preset === "edge" && <details className="advanced-section wide"><summary>Advanced</summary><div className="advanced-grid">
      <DynamicSliderNumber tooltip="Horizontal scale of each indicator relative to its configured Size." label="Width" range={rangeFor("width", legacyRange(geometry.width, geometry.widthStrengthLink, 5, 400))} min={5} max={400} step={1} value={geometry.width} suffix="%" onChange={(value) => setGeometry("width", value)} onRangeChange={(range) => setDynamicRange("width", range)} />
      <DynamicSliderNumber tooltip="Vertical scale of each indicator relative to its configured Size." label="Height" range={rangeFor("height", legacyRange(geometry.height, geometry.heightStrengthLink, 5, 400))} min={5} max={400} step={1} value={geometry.height} suffix="%" onChange={(value) => setGeometry("height", value)} onRangeChange={(range) => setDynamicRange("height", range)} />
      <DynamicSliderNumber className="wide" tooltip="Clockwise rotation of each indicator relative to its calculated Orientation." label="Rotation" range={rangeFor("rotation", legacyRange(geometry.rotation, geometry.rotationStrengthLink, -180, 180))} min={-180} max={180} step={1} value={geometry.rotation} suffix="°" onChange={(value) => setGeometry("rotation", value)} onRangeChange={(range) => setDynamicRange("rotation", range)} />
    </div></details>}
    {effect.preset !== "edge" && <details className="advanced-section wide"><summary>Advanced</summary><div className="advanced-grid">
    ...
    {effect.preset === "radar" && <details className="animation-section wide" open={animationOpen} onToggle={(event) => setAnimationOpen(event.currentTarget.open)}><summary>Animation <small>{effect.radar?.sweepType ?? "none"}</small></summary><div className="animation-grid">
      <label title="Animation mode; None displays all echoes continuously at full intensity."><Label>Mode</Label><select value={effect.radar?.sweepType ?? "none"} onChange={(event) => { const sweepType = event.target.value as "none" | "radial" | "angular"; setRadar({ sweepType, ...(sweepType === "radial" ? { sweepDirection: "outward" as const } : sweepType === "angular" ? { sweepDirection: "clockwise" as const } : {}) }); }}><option value="none">None</option><option value="radial">Radial</option><option value="angular">Angular</option></select></label>
      {(effect.radar?.sweepType ?? "none") !== "none" && <>
      <label title="Direction traveled by the detection line."><Label>Direction</Label><select value={effect.radar?.sweepDirection ?? "outward"} onChange={(event) => setRadar({ sweepDirection: event.target.value as NonNullable<ShaderEffectDefinitionV1["radar"]>["sweepDirection"] })}>{effect.radar?.sweepType === "angular" ? <><option value="clockwise">Clockwise</option><option value="counterclockwise">Counterclockwise</option></> : effect.radar?.sweepType === "radial" ? <><option value="outward">Outward</option><option value="inward">Inward</option></> : <><option value="outward">Outward</option><option value="inward">Inward</option><option value="clockwise">Clockwise</option><option value="counterclockwise">Counterclockwise</option></>}</select></label>
      <DynamicSliderNumber tooltip="Seconds for an animated echo to fade from full intensity to invisible." label="Echo fade" min={0.1} max={30} step={0.1} decimals={1} suffix="s" value={effect.radar?.echoFadeDuration ?? 3} range={rangeFor("echoFadeDuration")} onChange={(echoFadeDuration) => setRadar({ echoFadeDuration })} onRangeChange={(range) => setDynamicRange("echoFadeDuration", range)} />
      <DynamicSliderNumber tooltip="Length of the logarithmically fading trail, from none to the previous half-cycle." label="Sweep trail" min={0} max={100} step={5} suffix="%" value={effect.radar?.sweepTrail ?? 0} range={rangeFor("radarSweepTrail")} onChange={(sweepTrail) => setRadar({ sweepTrail })} onRangeChange={(range) => setDynamicRange("radarSweepTrail", range)} />
      <DynamicSliderNumber tooltip="Sweep cycles per second." label="Rate" range={rangeFor("animationRate", legacyRange(effect.animation?.rate ?? 1, effect.animation?.rateStrengthLink, 0, 10))} min={0} max={10} step={0.1} decimals={1} value={effect.animation?.rate ?? 1} onChange={(rate) => setAnimation({ rate })} onRangeChange={(range) => setDynamicRange("animationRate", range)} />
      <DynamicSliderNumber tooltip="Sweep opacity relative to the configured maximum Intensity." label="Depth" range={rangeFor("animationDepth", legacyRange(effect.animation?.depth ?? 0.35, effect.animation?.depthStrengthLink, 0, 1))} min={0} max={1} step={0.05} decimals={2} value={effect.animation?.depth ?? 0.35} onChange={(depth) => setAnimation({ depth })} onRangeChange={(range) => setDynamicRange("animationDepth", range)} />
      <DynamicSliderNumber className="wide" tooltip="Thickness of the moving detection line." label="Wave width" range={rangeFor("waveWidth", legacyRange(effect.animation?.waveWidth ?? 0.22, effect.animation?.waveWidthStrengthLink, 0, 1))} min={0} max={1} step={0.05} decimals={2} value={effect.animation?.waveWidth ?? 0.22} onChange={(waveWidth) => setAnimation({ waveWidth })} onRangeChange={(range) => setDynamicRange("waveWidth", range)} />
      </>}
    </div></details>}
    {effect.preset !== "radar" && effect.preset !== "grid" && <details className="animation-section wide" open={animationOpen} onToggle={(event) => setAnimationOpen(event.currentTarget.open)}><summary>Animation <small>{effect.animation?.mode ?? "none"}</small></summary><div className="animation-grid">
...
function DebugView({ rules, authority, pending, error, onTakeControl, onReleaseControl }: DebugViewProps) {
  return <section className="content-card debug-view"><h2>Local runtime</h2><p className="muted">Derived state from this client only. Nothing here is stored in scene metadata.</p><AuthorityCard authority={authority} pending={pending} error={error} onTakeControl={onTakeControl} onReleaseControl={onReleaseControl} />{rules.length === 0 ? <div className="notice">No active detector rules.</div> : rules.map((rule) => <article key={`${rule.detectorId}:${rule.ruleId}`}><h3>{rule.detectorName}</h3><code>{rule.signal} · {rule.detectionArea === "source-area" ? "source area" : `${rule.range.inner}–${rule.range.outer}`} · {rule.aggregation === "all" ? "all detected items" : "closest detected item"}</code><dl className="facts"><div><dt>Matches</dt><dd>{rule.matchingEmitterCount}</dd></div><div><dt>Active</dt><dd>{rule.activeEmitterCount}</dd></div></dl>{rule.detections.map((detection, index) => <div className="debug-detection" key={`${detection.emitterName}:${index}`}><strong>{detection.emitterName}</strong><span>{detection.distance.toFixed(2)} · strength {detection.strength.toFixed(3)}</span></div>)}{rule.effects.map((effect, index) => <div className="debug-effect" key={`${effect.effectId}:${effect.runtimeKey ?? index}`}><strong>{effect.providerId ? `${effect.providerId} · ${effect.actionId}` : effect.actionId ? `${effect.type === "mechanical" ? "state" : effect.type} · ${effect.actionId}` : effect.type === "mechanical" ? "state" : effect.type} · {effect.lifecycle}</strong><span>{effect.transition} · {effect.targetType} → {effect.targetName ?? "unresolved"}</span><span>{effect.audience ?? "GM authority"} · {effect.audienceMatch ? "execution client" : "not executing here"}</span><code>{effect.executionStatus ?? effect.localItemId ?? "inactive"}</code></div>)}</article>)}</section>;
}
