...
export default function App() {
  ...
  return <main className="app-shell">
    ...
    {showDebug && connection.role === "GM" ? <><div className="debug-emitter-labels"><button className="wide-button" disabled={!connection.sceneReady} onClick={toggleEmitterLabels}>{emitterLabelsVisible ? "Clear emitter labels" : "Label all emitters"}</button>{emitterLabelsVisible && <p className="muted" role="status">Showing {emitterLabelCount} local emitter label{emitterLabelCount === 1 ? "" : "s"}.</p>}</div><DebugView rules={debug} authority={authority} pending={authorityPending} error={authorityError} onTakeControl={() => sendAuthorityCommand("take-control")} onReleaseControl={() => sendAuthorityCommand("release-control")} /></> : selected && !showSettings && connection.role === "GM" ? <>
