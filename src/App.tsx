...
function EffectEditor({ effect, items, party, onSave, onChange, onDelete }: { effect: ShaderEffectDefinitionV1; items: Item[]; party: Player[]; onSave: () => void; onChange: (update: (effect: ShaderEffectDefinitionV1) => ShaderEffectDefinitionV1) => void; onDelete: () => void }) {
  ...
  return <div className="effect-card"><div className="section-title"><strong>{effect.preset[0].toUpperCase() + effect.preset.slice(1)}</strong><div className="effect-header-actions"><button className="mini-icon" title="Save this effect to your browser-local library." aria-label="Save effect to library" onClick={onSave}><SaveIcon /></button><label className="toggle"><input type="checkbox" checked={effect.enabled} onChange={(event) => onChange((value) => ({ ...value, enabled: event.target.checked }))} /> Enabled</label></div></div><div className="form-grid">
    ...
    <label className="toggle wide" title="Show this effect to every GM in addition to the selected audience."><input type="checkbox" checked={effect.alwaysIncludeGm ?? false} onChange={(event) => onChange((value) => ({ ...value, alwaysIncludeGm: event.target.checked }))} /> Always include GM</label>
    ...
    {effect.preset === "beam" && <SliderNumber tooltip="Angular width of the directional beam. Optionally link beam width to signal strength." label="Beam width" labelContent={<StrengthLinkLabel label="Beam width" value={effect.beamWidthStrengthLink} onChange={(beamWidthStrengthLink) => onChange((value) => ({ ...value, beamWidthStrengthLink }))} />} min={5} max={120} step={1} value={effect.beamWidth ?? 38} suffix="°" onChange={(beamWidth) => onChange((value) => ({ ...value, beamWidth }))} />}
    <div className="paired-controls wide"><SliderNumber tooltip="Horizontal scale of the effect; 100% matches the target width." label="Width" labelContent={<StrengthLinkLabel label="Width" value={geometry.widthStrengthLink} onChange={(value) => setGeometryLink("widthStrengthLink", value)} />} min={5} max={400} step={1} value={geometry.width} suffix="%" onChange={(value) => setGeometry("width", value)} /><SliderNumber tooltip="Vertical scale of the effect; 100% matches the target height." label="Height" labelContent={<StrengthLinkLabel label="Height" value={geometry.heightStrengthLink} onChange={(value) => setGeometryLink("heightStrengthLink", value)} />} min={5} max={400} step={1} value={geometry.height} suffix="%" onChange={(value) => setGeometry("height", value)} /></div>
    <SliderNumber tooltip={effect.preset === "beam" ? "Rotation offset from the automatically detected direction." : "Clockwise rotation of the effect's local axes."} label="Rotation" labelContent={<StrengthLinkLabel label="Rotation" value={geometry.rotationStrengthLink} onChange={(value) => setGeometryLink("rotationStrengthLink", value)} />} min={-180} max={180} step={1} value={geometry.rotation} suffix="°" onChange={(value) => setGeometry("rotation", value)} />
    <div className="paired-controls wide"><SliderNumber tooltip="Move the effect center horizontally as a percentage of target size." label="X offset" labelContent={<StrengthLinkLabel label="X offset" value={geometry.offsetXStrengthLink} onChange={(value) => setGeometryLink("offsetXStrengthLink", value)} />} min={-100} max={100} step={1} value={geometry.offsetX} suffix="%" onChange={(value) => setGeometry("offsetX", value)} /><SliderNumber tooltip="Move the effect center vertically as a percentage of target size." label="Y offset" labelContent={<StrengthLinkLabel label="Y offset" value={geometry.offsetYStrengthLink} onChange={(value) => setGeometryLink("offsetYStrengthLink", value)} />} min={-100} max={100} step={1} value={geometry.offsetY} suffix="%" onChange={(value) => setGeometry("offsetY", value)} /></div>
