...
import { StrengthLinkLabel } from "./components/StrengthLinkLabel";
...
const newRule = (): DetectionRuleV1 => ({ id: crypto.randomUUID(), enabled: true, signal: "", aggregation: "nearest", ignoreHidden: false, range: { outer: 60, inner: 5 }, falloff: "smoothstep", effects: [newEffect()] });
...
const effectDefaultName = (effect: EffectDefinitionV1) => effect.type === "shader" ? effect.preset === "beam" ? "Directional beam" : "Glow" : effect.type === "mechanical" ? effect.action === "face" ? "Face" : "Hide/Show" : INTEGRATION_CATALOG.find((provider) => provider.id === effect.providerId)?.displayName ?? "Integration effect";
...
function RuleEditor({ rule, index, unit, items, party, emanationEnabled, library, onSaveEffect, onChange, onEffect, onDelete }: RuleEditorProps) {
  ...
  return <article className="rule-card"><div className="section-title"><div className="card-heading-main"><button className="collapse-button" title={collapsed ? "Expand rule" : "Collapse rule"} aria-label={collapsed ? `Expand rule ${index + 1}` : `Collapse rule ${index + 1}`} aria-expanded={!collapsed} onClick={() => setCollapsed((value) => !value)}><CaretIcon collapsed={collapsed} /></button><EditableTitle as="h3" value={rule.name} fallback={`Rule ${index + 1}`} ariaLabel={`Rename rule ${index + 1}`} onChange={(name) => onChange((value) => ({ ...value, name }))} /></div><div className="rule-header-actions"><label className="toggle" title="Enable or disable this detection rule."><input type="checkbox" aria-label={`Enable rule ${index + 1}`} checked={rule.enabled} onChange={(event) => onChange((value) => ({ ...value, enabled: event.target.checked }))} /></label><button className="mini-icon danger delete-icon" title="Delete this detection rule." aria-label={`Delete rule ${index + 1}`} onClick={onDelete}><TrashIcon /></button></div></div>{!collapsed && <><div className="form-grid">
    <label title="Signal tag this rule listens for."><Label tooltip="Signal tag this rule listens for.">Signal</Label><input value={rule.signal} placeholder="Add signal…" onChange={(event) => onChange((value) => ({ ...value, signal: event.target.value }))} /></label>
...
function MechanicalEffectEditor({ effect, items, onSave, onChange, onDelete }: { effect: MechanicalEffectDefinitionV1; items: Item[]; onSave: () => void; onChange: (update: (effect: MechanicalEffectDefinitionV1) => MechanicalEffectDefinitionV1) => void; onDelete: () => void }) {
  ...
  const switchAction = (action: MechanicalEffectDefinitionV1["action"]) => onChange((current) => action === "face"
    ? { ...newFaceEffect(), id: current.id, name: current.name, enabled: current.enabled, target: current.target }
    : { ...newVisibilityEffect(), id: current.id, name: current.name, enabled: current.enabled, target: current.target });
...
function EffectEditor({ effect, items, party, onSave, onChange, onDelete }: { effect: ShaderEffectDefinitionV1; items: Item[]; party: Player[]; onSave: () => void; onChange: (update: (effect: ShaderEffectDefinitionV1) => ShaderEffectDefinitionV1) => void; onDelete: () => void }) {
  ...
  type GeometryNumberField = "offsetX" | "offsetY" | "innerRadius" | "outerRadius" | "width" | "height" | "rotation";
  type GeometryLinkField = "offsetXStrengthLink" | "offsetYStrengthLink" | "innerRadiusStrengthLink" | "outerRadiusStrengthLink" | "widthStrengthLink" | "heightStrengthLink" | "rotationStrengthLink";
  ...
  const setGeometryLink = (field: GeometryLinkField, value: "min" | "max" | undefined) => onChange((current) => ({ ...current, geometry: { ...resolveShaderGeometry(current), [field]: value } }));
  ...
  return <div className="effect-card"><div className="section-title"><div className="card-heading-main"><button className="collapse-button" title={collapsed ? "Expand effect" : "Collapse effect"} aria-label={collapsed ? `Expand ${title} effect` : `Collapse ${title} effect`} aria-expanded={!collapsed} onClick={() => setCollapsed((value) => !value)}><CaretIcon collapsed={collapsed} /></button><EditableTitle value={effect.name} fallback={defaultTitle} ariaLabel={`Rename ${defaultTitle} effect`} onChange={(name) => onChange((value) => ({ ...value, name }))} /></div><div className="effect-header-actions"><button className="mini-icon" title="Save this effect to your browser-local library." aria-label="Save effect to library" onClick={onSave}><SaveToBookIcon /></button><label className="toggle" title="Enable or disable this effect."><input type="checkbox" aria-label={`Enable ${title} effect`} checked={effect.enabled} onChange={(event) => onChange((value) => ({ ...value, enabled: event.target.checked }))} /></label><button className="mini-icon danger delete-icon" title="Delete effect" aria-label={`Delete ${title} effect`} onClick={onDelete}><TrashIcon /></button></div></div>{!collapsed && <><div className="form-grid">
    ...
    <SliderNumber tooltip="Effect opacity. With MAX enabled it scales with signal strength; disable MAX for constant intensity while active." label="Intensity" labelContent={<ToggleOptionLabel label="Intensity" option="Max" active={effect.intensityStrengthLinked ?? true} activeTitle="Use constant configured intensity while the effect is active." inactiveTitle="Scale from zero to the configured maximum using signal strength." onChange={(intensityStrengthLinked) => onChange((value) => ({ ...value, intensityStrengthLinked }))} />} min={0} max={2} step={0.05} decimals={2} value={effect.maxIntensity} onChange={(maxIntensity) => onChange((value) => ({ ...value, maxIntensity }))} />
    <SliderNumber tooltip="Feathering at the inner and outer edges. Optionally link softness to signal strength." label="Softness" labelContent={<StrengthLinkLabel label="Softness" value={effect.spreadStrengthLink} onChange={(spreadStrengthLink) => onChange((value) => ({ ...value, spreadStrengthLink }))} />} min={0.05} max={4} step={0.05} decimals={2} value={effect.spread} onChange={(spread) => onChange((value) => ({ ...value, spread }))} />
    {effect.preset === "beam" ? <div className="paired-controls wide"><SliderNumber tooltip="Radial distance where the beam begins; 100% is the target edge." label="Beam start" labelContent={<StrengthLinkLabel label="Beam start" value={geometry.innerRadiusStrengthLink} onChange={(value) => setGeometryLink("innerRadiusStrengthLink", value)} />} min={0} max={Math.max(0, geometry.outerRadius - 1)} step={1} value={geometry.innerRadius} suffix="%" onChange={(value) => setGeometry("innerRadius", value)} /><SliderNumber tooltip="Radial distance where the beam ends; 100% is the target edge." label="Beam end" labelContent={<StrengthLinkLabel label="Beam end" value={geometry.outerRadiusStrengthLink} onChange={(value) => setGeometryLink("outerRadiusStrengthLink", value)} />} min={geometry.innerRadius + 1} max={200} step={1} value={geometry.outerRadius} suffix="%" onChange={(value) => setGeometry("outerRadius", value)} /></div> : <><SliderNumber tooltip="Inner edge of the glow; 100% is the target edge." label="Inner radius" min={0} max={Math.max(0, geometry.outerRadius - 1)} step={1} value={geometry.innerRadius} suffix="%" onChange={(value) => setGeometry("innerRadius", value)} /><SliderNumber tooltip="Outer edge of the glow; 100% is the target edge." label="Outer radius" min={geometry.innerRadius + 1} max={200} step={1} value={geometry.outerRadius} suffix="%" onChange={(value) => setGeometry("outerRadius", value)} /></>}
    {effect.preset === "beam" && <SliderNumber className="wide" tooltip="Angular width of the directional beam. Optionally link beam width to signal strength." label="Beam width" labelContent={<StrengthLinkLabel label="Beam width" value={effect.beamWidthStrengthLink} onChange={(beamWidthStrengthLink) => onChange((value) => ({ ...value, beamWidthStrengthLink }))} />} min={5} max={120} step={1} value={effect.beamWidth ?? 38} suffix="°" onChange={(beamWidth) => onChange((value) => ({ ...value, beamWidth }))} />}
    <details className="advanced-section wide"><summary>Advanced</summary><div className="advanced-grid">
      <SliderNumber tooltip="Horizontal scale of the effect; 100% matches the target width." label="Width" labelContent={<StrengthLinkLabel label="Width" value={geometry.widthStrengthLink} onChange={(value) => setGeometryLink("widthStrengthLink", value)} />} min={5} max={400} step={1} value={geometry.width} suffix="%" onChange={(value) => setGeometry("width", value)} />
      <SliderNumber tooltip="Vertical scale of the effect; 100% matches the target height." label="Height" labelContent={<StrengthLinkLabel label="Height" value={geometry.heightStrengthLink} onChange={(value) => setGeometryLink("heightStrengthLink", value)} />} min={5} max={400} step={1} value={geometry.height} suffix="%" onChange={(value) => setGeometry("height", value)} />
      <SliderNumber tooltip="Move the effect center horizontally as a percentage of target size." label="X offset" labelContent={<StrengthLinkLabel label="X offset" value={geometry.offsetXStrengthLink} onChange={(value) => setGeometryLink("offsetXStrengthLink", value)} />} min={-100} max={100} step={1} value={geometry.offsetX} suffix="%" onChange={(value) => setGeometry("offsetX", value)} />
      <SliderNumber tooltip="Move the effect center vertically as a percentage of target size." label="Y offset" labelContent={<StrengthLinkLabel label="Y offset" value={geometry.offsetYStrengthLink} onChange={(value) => setGeometryLink("offsetYStrengthLink", value)} />} min={-100} max={100} step={1} value={geometry.offsetY} suffix="%" onChange={(value) => setGeometry("offsetY", value)} />
      <SliderNumber className="wide" tooltip={effect.preset === "beam" ? "Rotation offset from the automatically detected direction." : "Clockwise rotation of the effect's local axes."} label="Rotation" labelContent={<StrengthLinkLabel label="Rotation" value={geometry.rotationStrengthLink} onChange={(value) => setGeometryLink("rotationStrengthLink", value)} />} min={-180} max={180} step={1} value={geometry.rotation} suffix="°" onChange={(value) => setGeometry("rotation", value)} />
    </div></details>
    <details className="animation-section wide" open={animationOpen} onToggle={(event) => setAnimationOpen(event.currentTarget.open)}><summary>Animation <small>{effect.animation?.mode ?? "none"}</small></summary><div className="animation-grid">
      ...
      {(effect.animation?.mode ?? "none") !== "none" && <><SliderNumber tooltip="Animation cycles per second. Optionally link the rate to signal strength." label="Rate" labelContent={<StrengthLinkLabel label="Rate" value={effect.animation?.rateStrengthLink} onChange={(rateStrengthLink) => setAnimation({ rateStrengthLink })} />} min={0} max={10} step={0.1} decimals={1} value={effect.animation?.rate ?? 1} onChange={(rate) => setAnimation({ rate })} /><SliderNumber tooltip="Difference between the dim and bright animation phases. Optionally link depth to signal strength." label="Depth" labelContent={<StrengthLinkLabel label="Depth" value={effect.animation?.depthStrengthLink} onChange={(depthStrengthLink) => setAnimation({ depthStrengthLink })} />} min={0} max={1} step={0.05} decimals={2} value={effect.animation?.depth ?? 0.35} onChange={(depth) => setAnimation({ depth })} /></>}
      {effect.animation?.mode === "radial-pulse" && <SliderNumber className="wide" tooltip="Thickness of the traveling radial band. Optionally link wave width to signal strength." label="Wave width" labelContent={<StrengthLinkLabel label="Wave width" value={effect.animation.waveWidthStrengthLink} onChange={(waveWidthStrengthLink) => setAnimation({ waveWidthStrengthLink })} />} min={0.05} max={1} step={0.05} decimals={2} value={effect.animation.waveWidth ?? 0.22} onChange={(waveWidth) => setAnimation({ waveWidth })} />}
    </div></details>
