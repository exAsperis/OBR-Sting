...
const newFaceEffect = (): MechanicalEffectDefinitionV1 => ({ id: crypto.randomUUID(), type: "mechanical", enabled: true, action: "face", target: { type: "detector" }, faceAngle: 0, speed: 180 });
...
const effectTypeLabel = (effect: EffectDefinitionV1) => effect.type === "shader" ? effect.preset : effect.type === "mechanical" ? "Face" : INTEGRATION_CATALOG.find((provider) => provider.id === effect.providerId)?.displayName ?? effect.providerId;
...
export default function App() {
  ...
  const saveEffectToLibrary = (effect: EffectDefinitionV1) => {
    const suggested = effect.type === "shader" ? effect.preset === "beam" ? "Directional beam" : "Glow" : effect.type === "mechanical" ? "Face" : INTEGRATION_CATALOG.find((provider) => provider.id === effect.providerId)?.displayName ?? "Integration effect";
