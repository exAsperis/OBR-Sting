...
import type { DebugRuleState, DetectionRuleV1, DetectorMetadataV1, EffectAudienceV1, EffectDefinitionV1, EffectTargetV1, EmanationEffectDefinitionV1, EmitterMetadataV1, ShaderEffectDefinitionV1 } from "./types";
...
const newEmanationEffect = (): EmanationEffectDefinitionV1 => ({ id: crypto.randomUUID(), type: "emanation", enabled: true, target: { type: "detector" }, audience: { type: "everyone" }, presetName: "Default", removeAllOnDeactivate: false });
...
export default function App() {
  ...
  return <main className="app-shell">
    ...
    {showDebug ? <DebugView rules={debug} /> : selected && connection.role === "GM" ? <>
      ...
      <section className="content-card integration-card"><div className="section-title"><div><h2>Auras and Emanations</h2><p className="muted">Allow Sting to trigger named presets through the installed extension.</p></div><label className="toggle"><input type="checkbox" checked={emanationEnabled} onChange={(event) => toggleEmanation(event.target.checked)} /> Enabled</label></div></section>
...
function RuleEditor({ rule, index, unit, items, party, emanationEnabled, onChange, onEffect, onDelete }: RuleEditorProps) {
  ...
  </div><div className="effects-heading"><h4>Effects <span>{rule.effects.length}</span></h4></div>{rule.effects.map((effect, effectIndex) => effect.type === "shader" ? <EffectEditor key={effect.id} effect={effect} items={items} party={party} onChange={(update) => onEffect(effectIndex, (current) => current.type === "shader" ? update(current) : current)} onDelete={() => onChange((value) => ({ ...value, effects: value.effects.filter((_, i) => i !== effectIndex) }))} /> : <EmanationEffectEditor key={effect.id} effect={effect} items={items} onChange={(update) => onEffect(effectIndex, (current) => current.type === "emanation" ? update(current) : current)} onDelete={() => onChange((value) => ({ ...value, effects: value.effects.filter((_, i) => i !== effectIndex) }))} />)}<div className="card-actions"><button onClick={() => onChange((value) => ({ ...value, effects: [...value.effects, newEffect()] }))}>+ Sting effect</button>{emanationEnabled && <button onClick={() => onChange((value) => ({ ...value, effects: [...value.effects, newEmanationEffect()] }))}>+ A&amp;E preset</button>}<button className="danger" onClick={onDelete}>Delete rule</button></div></article>;
}

function EmanationEffectEditor({ effect, items, onChange, onDelete }: { effect: EmanationEffectDefinitionV1; items: Item[]; onChange: (update: (effect: EmanationEffectDefinitionV1) => EmanationEffectDefinitionV1) => void; onDelete: () => void }) {
  const setTarget = (type: EffectTargetV1["type"]) => onChange((value) => ({ ...value, target: type === "specific-item" ? { type, itemId: items[0]?.id ?? "" } : { type } }));
  return <div className="effect-card emanation-card"><div className="section-title"><strong>A&amp;E preset</strong><label className="toggle"><input type="checkbox" checked={effect.enabled} onChange={(event) => onChange((value) => ({ ...value, enabled: event.target.checked }))} /> Enabled</label></div><div className="form-grid">
    <label className="wide"><Label>Preset or group name</Label><input value={effect.presetName} onChange={(event) => onChange((value) => ({ ...value, presetName: event.target.value }))} /></label>
  ...
  </div><label className="cleanup-option"><input type="checkbox" checked={effect.removeAllOnDeactivate} onChange={(event) => onChange((value) => ({ ...value, removeAllOnDeactivate: event.target.checked }))} /><span><strong>Remove auras when inactive</strong><small>Warning: the A&amp;E API removes every A&amp;E aura from this target, including auras not created by Sting.</small></span></label><button className="danger text-button" onClick={onDelete}>Delete effect</button></div>;
}
...
function EffectEditor({ effect, items, party, onChange, onDelete }: { effect: ShaderEffectDefinitionV1; items: Item[]; party: Player[]; onChange: (update: (effect: ShaderEffectDefinitionV1) => ShaderEffectDefinitionV1) => void; onDelete: () => void }) {
  ...
  return <div className="effect-card"><div className="section-title"><strong>{effect.preset[0].toUpperCase() + effect.preset.slice(1)}</strong><label className="toggle"><input type="checkbox" checked={effect.enabled} onChange={(event) => onChange((value) => ({ ...value, enabled: event.target.checked }))} /> Enabled</label></div><div className="form-grid">
    <label><Label>Preset</Label><select value={effect.preset} onChange={(event) => onChange((value) => { const preset = event.target.value as ShaderEffectDefinitionV1["preset"]; return { ...value, preset, geometry: DEFAULT_GEOMETRY[preset] }; })}><option value="glow">Glow</option><option value="pulse">Pulse</option><option value="flicker">Flicker</option><option value="outline">Outline</option><option value="beam">Directional beam</option></select></label>
    <label><Label>Target</Label><select value={effect.target.type} onChange={(event) => setTarget(event.target.value as EffectTargetV1["type"])}><option value="detector">Detector</option><option value="parent">Parent</option><option value="carrier">Carrier</option><option value="detected-emitter">Detected emitter</option><option value="specific-item">Specific item</option></select></label>
    {effect.target.type === "specific-item" && <label className="wide"><Label>Specific item</Label><select value={effect.target.itemId} onChange={(event) => onChange((value) => ({ ...value, target: { type: "specific-item", itemId: event.target.value } }))}>{items.map((item) => <option key={item.id} value={item.id}>{item.name || item.id}</option>)}</select></label>}
    ...
    <p className="field-hint">100% reaches the target edge. Values above 100% extend beyond it.</p>
