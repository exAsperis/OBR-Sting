...
const effectTypeLabel = (effect: EffectDefinitionV1) => effect.type === "shader" ? effect.preset : INTEGRATION_CATALOG.find((provider) => provider.id === effect.providerId)?.displayName ?? effect.providerId;
...
export default function App() {
  ...
  const saveEffectToLibrary = (effect: EffectDefinitionV1) => {
    const suggested = effect.type === "shader" ? effect.preset === "beam" ? "Directional beam" : "Glow" : INTEGRATION_CATALOG.find((provider) => provider.id === effect.providerId)?.displayName ?? "Integration effect";
...
function RuleEditor({ rule, index, unit, items, party, emanationEnabled, library, onSaveEffect, onChange, onEffect, onDelete }: RuleEditorProps) {
  ...
  </div><div className="effects-heading"><h4>Effects <span>{rule.effects.length}</span></h4><div className="effect-add-actions" aria-label="Add effect"><button className="effect-glyph" title="Add a native Sting effect." aria-label="Add a native Sting effect" onClick={() => onChange((value) => ({ ...value, effects: [...value.effects, newEffect()] }))}><img src="./icon.svg" alt="" /></button><button className={`effect-glyph${libraryOpen ? " active" : ""}`} title={library.length ? "Add an effect from your browser-local library." : "Your browser-local effects library is empty."} aria-label="Add an effect from the effects library" disabled={!library.length} onClick={() => setLibraryOpen((value) => !value)}><BookIcon /></button>{enabledProviders.map((provider) => <button className="effect-glyph" key={provider.id} title={`Add ${provider.displayName} effect.`} aria-label={`Add ${provider.displayName} effect`} onClick={() => onChange((value) => ({ ...value, effects: [...value.effects, createIntegrationEffect(provider.id, provider.actions[0].id)] }))}><img src={provider.iconUrl} alt="" /></button>)}</div></div>{libraryOpen && <div className="library-picker">{library.map((entry) => <button key={entry.id} title={`Add ${entry.name} to this rule.`} onClick={() => { onChange((value) => ({ ...value, effects: [...value.effects, instantiateLibraryEffect(entry)] })); setLibraryOpen(false); }}><span>{entry.name}</span><small>{entry.effect.type === "shader" ? entry.effect.preset : "integration"}</small></button>)}</div>}{rule.effects.map((effect, effectIndex) => effect.type === "shader" ? <EffectEditor key={effect.id} effect={effect} items={items} party={party} onSave={() => onSaveEffect(effect)} onChange={(update) => onEffect(effectIndex, (current) => current.type === "shader" ? update(current) : current)} onDelete={() => onChange((value) => ({ ...value, effects: value.effects.filter((_, i) => i !== effectIndex) }))} /> : <IntegrationEffectEditor key={effect.id} effect={effect} items={items} providerEnabled={emanationEnabled} onSave={() => onSaveEffect(effect)} onChange={(update) => onEffect(effectIndex, (current) => current.type === "integration" ? update(current) : current)} onDelete={() => onChange((value) => ({ ...value, effects: value.effects.filter((_, i) => i !== effectIndex) }))} />)}</article>;
}
...
function DebugView({ rules }: { rules: DebugRuleState[] }) {
  return <section className="content-card debug-view"><h2>Local runtime</h2><p className="muted">Derived state from this client only. Nothing here is stored in scene metadata.</p>{rules.length === 0 ? <div className="notice">No active detector rules.</div> : rules.map((rule) => <article key={`${rule.detectorId}:${rule.ruleId}`}><h3>{rule.detectorName}</h3><code>{rule.signal} · {rule.aggregation === "all" ? "all in range" : "closest"} · {rule.range.outer}</code><dl className="facts"><div><dt>Matches</dt><dd>{rule.matchingEmitterCount}</dd></div><div><dt>Active</dt><dd>{rule.activeEmitterCount}</dd></div></dl>{rule.detections.map((detection, index) => <div className="debug-detection" key={`${detection.emitterName}:${index}`}><strong>{detection.emitterName}</strong><span>{detection.distance.toFixed(2)} · strength {detection.strength.toFixed(3)}</span></div>)}{rule.effects.map((effect, index) => <div className="debug-effect" key={`${effect.effectId}:${effect.runtimeKey ?? index}`}><strong>{effect.providerId ? `${effect.providerId} · ${effect.actionId}` : effect.type} · {effect.lifecycle}</strong><span>{effect.transition} · {effect.targetType} → {effect.targetName ?? "unresolved"}</span><span>{effect.audience} · {effect.audienceMatch ? "execution client" : "not executing here"}</span><code>{effect.executionStatus ?? effect.localItemId ?? "inactive"}</code></div>)}</article>)}</section>;
}
